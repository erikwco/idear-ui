...
## Decisiones
1. **Tailwind v3 → v4.** **shorts ✅ migrado** a TW4 + React Router v7 + React 19 (receta probada en `docs/migration-tw4-rr7.md`); consume el `theme-idear.css` del registro tal cual. **qrUI** sigue en TW3 (es Go/templates, migración acotada solo de pipeline CSS) y consume la adaptación TW3 (`L C H` pelado + `<alpha-value>`). Los tokens son válidos como variables CSS en ambas versiones. Las demás apps en TW3/Remix v1 pueden seguir el mismo playbook.
